...
- DLC/에디션 관련 할인은 연결된 본편과 스토어가 같은 경우에만 카드에 표시합니다.
- 외부 `dealID`가 바뀌더라도 같은 게임/스토어의 동일 할인이 최근 기록과 겹치면 중복 전송하지 않습니다.
...
- 오늘 종료되는 할인은 일반 알림과 종료 알림으로 중복 전송하지 않고, 빨간 카드와 `오늘 할인 종료` 제목으로 한 번만 강조합니다.
