...
export async function findBestCurrentDeal(query) {
  const games = await searchCheapSharkGames(query, 10);
  const game = [...games].sort((a, b) => {
  ...
  const steamDetails = deal.steamAppID ? await fetchSteamAppDetails(deal.steamAppID) : null;
